...
void BatchedThreadedNnet3CudaPipelineCustomized::Initialize(
    const fst::Fst<fst::StdArc> &decode_fst, const nnet3::AmNnetSimple &am_nnet,
    const TransitionModel &trans_model) {
  KALDI_LOG << "BatchedThreadedNnet3CudaPipelineCustomized Initialize with "
  ...
  for (int i = 0; i < config_.num_control_threads; i++) {
    thread_contexts_[i] =
        std::thread(&BatchedThreadedNnet3CudaPipelineCustomized::ExecuteWorker, this, i);
  }
...
void BatchedThreadedNnet3CudaPipelineCustomized::Finalize() {
...
bool BatchedThreadedNnet3CudaPipelineCustomized::isFinished(const std::string &key) {
...
void BatchedThreadedNnet3CudaPipelineCustomized::CloseDecodeHandle(const std::string &key) {
  tasks_lookup_mutex_.lock();
  auto it = tasks_lookup_.find(key);
  KALDI_ASSERT(it != tasks_lookup_.end());
  tasks_lookup_mutex_.unlock();
...
void BatchedThreadedNnet3CudaPipelineCustomized::OpenDecodeHandle(const std::string &key,
  ...
  work_pool_->enqueue(&BatchedThreadedNnet3CudaPipelineCustomized::ComputeOneFeature, this,
                      task);
}

// Add a decoding task to the decoder with a passed array of samples
void BatchedThreadedNnet3CudaPipelineCustomized::OpenDecodeHandle(
...
bool BatchedThreadedNnet3CudaPipelineCustomized::GetRawLattice(const std::string &key,
                                               Lattice *lat) {
...
bool BatchedThreadedNnet3CudaPipelineCustomized::GetLattice(const std::string &key,
                                            CompactLattice *clat) {
...
void BatchedThreadedNnet3CudaPipelineCustomized::PrefixTaskToPendingTaskQueue(TaskState *task) {
  tasks_mutex_.lock();
  if (NumPendingTasks() == config_.max_pending_tasks) {
    // task queue is full launch a new thread to add this task and exit to make
    // room for other work
    work_pool_->enqueue(&BatchedThreadedNnet3CudaPipelineCustomized::PrefixTaskToPendingTaskQueue,
                        this, task);
  } else {
    // there is room so let's add it
    // insert into pending task queue
    auto new_task_front_ = ((tasks_front_ - 1) + (config_.max_pending_tasks + 1)) % (config_.max_pending_tasks + 1);
    pending_task_queue_[new_task_front_] = task;
    tasks_front_ = new_task_front_;
  }
  tasks_mutex_.unlock();
}

// Adds task to the PendingTaskQueue
void BatchedThreadedNnet3CudaPipelineCustomized::AddTaskToPendingTaskQueue(TaskState *task) {
  tasks_add_mutex_.lock();
  if (NumPendingTasks() == config_.max_pending_tasks) {
    // task queue is full launch a new thread to add this task and exit to make
    // room for other work
    work_pool_->enqueue(&BatchedThreadedNnet3CudaPipelineCustomized::AddTaskToPendingTaskQueue,
                        this, task);
  } else {
    // there is room so let's add it
    // insert into pending task queue
...
// batch.
void BatchedThreadedNnet3CudaPipelineCustomized::AquireAdditionalTasks(
...
void BatchedThreadedNnet3CudaPipelineCustomized::ComputeBatchNnet(
...
void BatchedThreadedNnet3CudaPipelineCustomized::ComputeOneFeature(TaskState *task_) {
...
void BatchedThreadedNnet3CudaPipelineCustomized::AllocateDecodables(
...
void BatchedThreadedNnet3CudaPipelineCustomized::RemoveCompletedChannels(
    CudaDecoder &cuda_decoder, ChannelState &channel_state,
    std::vector<CudaDecodableInterface *> &decodables,
    std::vector<TaskState *> &tasks) {
...
//
void BatchedThreadedNnet3CudaPipelineCustomized::PostDecodeProcessing(
    CudaDecoder &cuda_decoder, ChannelState &channel_state,
    std::vector<CudaDecodableInterface *> &decodables,
    std::vector<TaskState *> &tasks) {
  std::vector<ChannelId> &channels = channel_state.channels;
  ...
  if (config_.determinize_lattice) {
    ...
    // finished.
    for (int i = channels.size(); i < tasks.size(); i++) {
      work_pool_->enqueue(&BatchedThreadedNnet3CudaPipelineCustomized::DeterminizeOneLattice,
                          this, tasks[i]);
    }
    nvtxRangePop();
  } else {
...
void BatchedThreadedNnet3CudaPipelineCustomized::DeterminizeOneLattice(TaskState *task) {
...
void BatchedThreadedNnet3CudaPipelineCustomized::ExecuteWorker(int threadId) {
