...
VFSProvider::VFSProvider(std::vector<uint8_t>&& mem) : memory(std::move(mem)) {
  memset(&zip_archive, 0, sizeof(zip_archive));

  mz_bool status =
      mz_zip_reader_init_mem(&zip_archive, memory.data(), memory.size(), 0);
  if (!status) {
  ...
  for (size_t i = 0; i < num_file; i++) {
    ...
    //           << "\n\tuncompressed size:" << stat.m_uncomp_size << std::endl;
  }
}

VFSProvider::~VFSProvider() { mz_zip_reader_end(&zip_archive); }
...
VFSProvider* Get() { return ::VFS::provider; }

void Set(VFSProvider* provider) { ::VFS::provider = provider; }

void Unset() { ::VFS::provider = nullptr; }
